...
document.querySelector("button").addEventListener("click",()=>
{
   ...
   toAppend+=`</tr></table></div><div><button id="submit" onclick="submit()" disabled>Submit</button><input id="pass" type="password" READONLY size="1"/>`;
   document.getElementById("main").innerHTML=toAppend;
...
function submit(){
   ...
   // alert(pass)
    if (pass==users[0].password) {
       ...
       var valuetry=3-itry;
       //alert("ok:"+valuetry);
        alert(valuetry+ " try left");
        if (valuetry==0){
        itry=0;alert("call the police");
        }
    }
